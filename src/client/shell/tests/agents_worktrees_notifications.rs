...
fn agent_group_fixture() -> ClientShellState {
    let mut projected = snapshot();
...
#[test]
fn agent_panel_groups_same_pane_agents_under_one_header() {
    let mut state = agent_group_fixture();
...
#[test]
fn agent_panel_toggle_collapses_group_and_shows_aggregate_severity() {
    let mut state = agent_group_fixture();
    frame_text(&mut state);
    let header = state.hits.agent_group_toggles[0].0;
    click(&mut state, (header.x, header.y));

    assert!(state.collapsed_agent_groups.contains("pane_1"));
    let text = frame_text(&mut state);
    assert!(!text.contains("pi one"), "frame: {text}");
    assert!(!text.contains("pi two"), "frame: {text}");
    assert!(text.contains("▸ editor"), "frame: {text}");
    ...
    let frame = state.compose(106, 30).expect("agent sidebar frame");
    let buffer = frame.to_ratatui_buffer().expect("agent sidebar buffer");
    let (x, y) = cell_symbol_position(&frame, state.hits.agent_group_toggles[0].0, "×");
    let icon = buffer.cell((x, y)).expect("aggregate status cell");
    assert_eq!(icon.symbol(), "×");
}

#[test]
fn agent_panel_toggle_expands_group_again() {
    let mut state = agent_group_fixture();
    frame_text(&mut state);
    let header = state.hits.agent_group_toggles[0].0;
    click(&mut state, (header.x, header.y));
    frame_text(&mut state);
    let collapsed_header = state.hits.agent_group_toggles[0].0;
    click(&mut state, (collapsed_header.x, collapsed_header.y));
